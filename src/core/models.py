...
class Publisher(models.Model):
    ...
    name = models.CharField(
        max_length=300,
        help_text=(
            'Publisher like Ubiquity Press or University of California Press'
        )
    )
    doi_prefix = models.CharField(
        max_length=20,
        help_text='Publisher\'s CrossRef prefix',
    )
...
class Publication(models.Model):
    ...
    identifier = models.CharField(
        max_length=200,
        help_text='Should be a doi, eg. 10.5334/cg.aa',
    )
    canonical_url = models.URLField(
        max_length=2000,
        help_text='Full URL with FQDN excluding http://',
    )
    canonical_url_two = models.URLField(
        max_length=2000,
        help_text='Full URL with FQDN excluding http://',
        blank=True,
        null=True,
...
class Wikipedia(models.Model):
    ...
    def __repr__(self):
        return u'<Wikipedia Citation %s %s>' % (
            self.publication.title, self.doi
        )
