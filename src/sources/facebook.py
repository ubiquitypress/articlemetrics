...
def query_graph_api(token):
    ...
    api.fql({
                'query': u"SELECT url, share_count, like_count, comment_count, click_count, total_count FROM link_stat WHERE url = '%{query_url}'"})
...
def query_links(publication):
    ...
    if d_request.status_code == 200:
        facebook_counts['share_count'] = facebook_counts[
                                             'share_count'] + d_dict.get(
            'share_count', 0)
        facebook_counts['like_count'] = facebook_counts[
                                            'like_count'] + d_dict.get(
            'like_count', 0)
        facebook_counts['comment_count'] = facebook_counts[
                                               'comment_count'] + d_dict.get(
            'comment_count', 0)
        facebook_counts['click_count'] = facebook_counts[
                                             'click_count'] + d_dict.get(
            'click_count', 0)
        facebook_counts['total_count'] = facebook_counts[
                                             'total_count'] + d_dict.get(
            'total_count', 0)

    if publication.canonical_url_two and ojs_request.status_code == 200:
        facebook_counts['share_count'] = facebook_counts[
                                             'share_count'] + ojs_dict.get(
            'share_count', 0)
        facebook_counts['like_count'] = facebook_counts[
                                            'like_count'] + ojs_dict.get(
            'like_count', 0)
        facebook_counts['comment_count'] = facebook_counts[
                                               'comment_count'] + ojs_dict.get(
            'comment_count', 0)
        facebook_counts['click_count'] = facebook_counts[
                                             'click_count'] + ojs_dict.get(
            'click_count', 0)
        facebook_counts['total_count'] = facebook_counts[
                                             'total_count'] + ojs_dict.get(
            'total_count', 0)
